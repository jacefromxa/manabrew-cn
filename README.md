...
少数未翻译卡牌（约 900 张）悬停时回退到 [mtgch.com API](https://mtgch.com/api/v1/docs)，结果自动缓存到本地。此外，若某张**本地牌**的规则文本、费用或攻防任一字段缺失（如新系列中 MTGJSON 名称匹配不上的生物），也会在悬停时后台向 mtgch API 补齐缺失字段——本地已有的卡名/文本/类别不会被覆盖，每张牌仅首次请求一次、结果长期缓存。
